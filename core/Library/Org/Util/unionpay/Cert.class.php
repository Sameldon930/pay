...
// 	    $cert->certId = $certId;
    ...
    public static function verifyAndGetVerifyCert($certBase64String,$mch_id){
    	$logger = LogUtil::getLogger($mch_id);
    	
    	if (array_key_exists($certBase64String, CertUtil::$verifyCerts510)){
    		return CertUtil::$verifyCerts510[$certBase64String];
    	}
		if (SDKConfig::getSDKConfig($mch_id)->middleCertPath === null || SDKConfig::getSDKConfig($mch_id)->rootCertPath === null){
		...
		if(strtolower(SDKConfig::getSDKConfig($mch_id)->ifValidateCNName) == "true"){
			if (COMPANY != $cn){
		...
		$result = openssl_x509_checkpurpose($certBase64String, X509_PURPOSE_ANY, array(SDKConfig::getSDKConfig($mch_id)->rootCertPath, SDKConfig::getSDKConfig($mch_id)->middleCertPath));
		if($result === FALSE){
    ...
    private static function initVerifyCerts($cert_dir=null,$mch_id) {

    	if( $cert_dir == null ) {
    		$cert_dir = SDKConfig::getSDKConfig($mch_id)->validateCertDir;
    	}
        ...
        while ($file = readdir($handle)) {
            ...
            if (is_file($filePath)) {
                if (pathinfo($file, PATHINFO_EXTENSION) == 'cer') {
                    ...
                    if(!openssl_x509_read($x509data)){
                    	$logger->LogInfo($filePath . " openssl_x509_read fail。");
                    	continue;
                    }
