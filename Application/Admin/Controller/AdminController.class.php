...
class AdminController extends BaseController
{
    ...
    public function addAdmin(){
        if(IS_POST){
            ...
            if ($data['mobile']){
                //手机号开头有86
                if (strpos($data['mobile'],'86') !== false){
                    $mobile = substr($data['mobile'],2);
                    $res = preg_match("/^((13[0-9])|(14[5,7])|(15[0-3,5-9])|(17[0,3,5-8])|(18[0-9])|166|198|199|(147))\\d{8}$/",$mobile);
                    if ($res == '0' && $res == false){
                        $this->ajaxReturn(['status'=>0,'msg'=>'手机号格式不正确!']);
                    }

                }else{
                    $res = preg_match("/^((13[0-9])|(14[5,7])|(15[0-3,5-9])|(17[0,3,5-8])|(18[0-9])|166|198|199|(147))\\d{8}$/",$data['mobile']);
                    if ($res == '0' && $res == false){
                        $this->ajaxReturn(['status'=>0,'msg'=>'手机号格式不正确!']);
                    }
