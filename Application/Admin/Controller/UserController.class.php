...
class UserController extends BaseController
{
    public function __construct()
    {
        parent::__construct();
        //通道
        $field = array('c.id', 'c.code', 'c.title', 'c.paytype', 'c.status', 'c.payment_product_id', 'c.product_id', 'pp.name');
        $channels = M('Channel')->alias("c")->join("__PAYMENT_PRODUCT__ as pp ON pp.id = c.payment_product_id")->where(['c.status' => 1])->field($field)->order("c.payment_product_id desc")
            ->select();
    ...
    //编辑用户支付产品费率(针对支付产品设置的费率)
    public function userPaymentProductRateEdit()
    {
        $userid = I('get.uid', 0, 'intval');
        //通道编码
        ...
        $where = [];
        if (I("product/d", 0)) $where["pid"] = I("product/d");
        if (I("payment_product/d", 0)) $where["id"] = I("payment_product/d");

        $payment_product = M("payment_product")->where($where)->order('id desc')->select();
        if ($payment_product) {
            foreach ($payment_product as $key => $item) {
                $_tmpData[$item['pid']][] = $item;
            }
        }
        //找出当前用户设置所有支付产品费率
        $user_payment_product_rate = M("user_payment_product_rate")->where(["userid" => $userid])->select();
        $user_rate = array_column($user_payment_product_rate, "rate", "payment_product_id");
        $this->assign(array(
            'products' => $_tmpData,
            'user_rate' => $user_rate,
            "banklist" => $banklist,
    ...
    function getPaymentProduct()
    {
        ...
        if (IS_AJAX) {
            ...
            if ($product_id) {
                try {
                    $data = M('payment_product')->where(["pid" => $product_id])->select();
                    $info = [
                        'status' => 1,
                        'msg' => 'ok',
                        'data' => $data,
    ...
    //保存用户设置的支付产品费率
    public function savePaymentProductRate()
    {
        $userid = I("userid");
        $payment_product_rate = I("payment_product_rate/a");
        $time = time();
        //构造批量插入或更新的数据
        foreach ($payment_product_rate as $k => $v) {
            foreach ($v as $key => $value) {
                $key_arr[] = $key;
                $data_insert[] = [
                    "payment_product_id" => $key,
        ...
        //删除该用户的费率,重新插入数据
        M("user_payment_product_rate")->where(["userid" => $userid,'payment_product_id'=>['in',$key_arr]])->delete();
        M("user_payment_product_rate")->addAll($data_insert, [], true);
        $this->ajaxReturn(['status' => 1]);
    }
    ...
    public function saveUserRate()
    {
        if (IS_POST) {
            ...
            foreach ($rows as $key => $item) {
                ...
                if ($item['t0feilv'] > 0 && $item['t0feilv'] > $item['t0fengding']) {
                    $this->ajaxReturn(['status' => 0, 'msg' => '【' . $product['name'] . '】T+0封顶费率不得低于T+0运营费率']);
                }
                if ($item['feilv'] > 0 && $item['feilv'] > $item['fengding']) {
                    $this->ajaxReturn(['status' => 0, 'msg' => '【' . $product['name'] . '】T+1封顶费率不得低于T+1运营费率']);
                }
                $rates = M('Userrate')->where(['userid' => $userid, 'payapiid' => $key])->find();
                $productUser = M('Product_user')->where(['pid' => $key])->find();
                        ...
                        if (!empty($channel)) {
                            if (!empty($channel_account_list)) {
                                foreach ($channel_account_list as $k => $v) {
                                    if ($item['t0feilv'] < $v['rate']) {
                                        $this->ajaxReturn(['status' => 0, 'msg' => '【' . $channel['title'] . '】T+0运营费率不得低于渠道子账号【' . $v['title'] . '】成本费率：' . ($v['rate'] * 1000) . '‰']);
                                    }
                        ...
                        if (!empty($temp_weights)) {
                            foreach ($temp_weights as $k => $v) {
                                ...
                                if (!empty($channel)) {
                                    if (!empty($channel_account_list)) {
                                        foreach ($channel_account_list as $k => $v) {
                                            if ($item['t0feilv'] > 0 && $item['t0feilv'] < $v['rate']) {
                                                $this->ajaxReturn(['status' => 0, 'msg' => '【' . $channel['title'] . '】T+0运营费率不得低于渠道子账号【' . $v['title'] . '】成本费率：' . ($v['rate'] * 1000) . '‰']);
                                            }
                                        ...
                                        if ($item['t0feilv'] > 0 && $item['t0feilv'] < $channel['rate']) {
                                            $this->ajaxReturn(['status' => 0, 'msg' => '【' . $channel['title'] . '】T+0运营费率不得低于渠道成本费率：' . ($channel['rate'] * 1000) . '‰']);
                                        }
                                        if ($item['feilv'] > 0 && $rows['feilv'] < $channel['rate']) {
    ...
    //编辑用户通道
    public function editUserProduct2()
    {
        $userid = I('get.uid', 0, 'intval');
        //系统产品列表
        $products = M('Product')
        ...
        //用户产品列表
//        $field = array('p.id','p.name','p.status','p.paytype','pp.id as payment_product_id','pp.name as payment_product_name');
//        $products = M('Product')->alias("p")->join("__PAYMENT_PRODUCT__ as pp On pp.pid = p.id")->where(['p.isdisplay' => 1])->field($field)->select();
        $userprods = M('Product_user')->where(['userid' => $userid])->select();
        if ($userprods) {
            foreach ($userprods as $key => $item) {
                $_tmpData[$item['pid']] = $item;
        ...
        $pid = array_column($payment_product, "pid", "id");
        foreach ($pid as $k => $v) {
            $pid_arr[$v][] = $k;
        }
        //重组产品列表
        ...
        if ($products) {
            foreach ($products as $key => $item) {
                $products[$key]['payment_product_id'] = $pid_arr[$item['id']] ? $pid_arr[$item['id']] : [];
                $products[$key]['status'] = $_tmpData[$item['id']]['status'];
                $products[$key]['channel'] = $_tmpData[$item['id']]['channel'];
                $products[$key]['polling'] = $_tmpData[$item['id']]['polling'];
                //权重
        ...
        $channels = M('Channel')
            ->where(['status' => 1])
            ->field('id,code,title,paytype,status,payment_product_id')
            ->order("payment_product_id desc")
            ->select();

        foreach ($channels as $k => $v) {
            $new_channels[$v['payment_product_id']][] = $v;
        }
        $this->assign(array(
            'products' => $products,
        ...
        foreach ($channelList as $k => $v) {
            foreach ($channelAccountList as $k1 => $v1) {
                if ($v1['channel_id'] == $v['id']) {
                    $list[$v['title']][] = $v1;
                    $list1[$v['title']]['mychannel_id'] = $v1['channel_id'];
                }
            }
        }

        $channel_control_list = M('user_channel_control')->field('id,channel_ids,type')->where(['userid' => $uid])->select();
        foreach ($channel_control_list as $k1 => $v1) {
            foreach ($list1 as $key => $value) {
                if ($v1['channel_ids'] == $value['mychannel_id']) {
                    $list1[$key]['type'] = $v1['type'];
                }
    ...
    {
        $uid = I('request.userid', 0, 'intval');
        $channel_ids = I('request.id');
        $type = I('request.type');
        if (empty($channel_ids)) {
            $this->ajaxReturn(['status' => 0, 'msg' => '没有选择子账号']);
        }
        $channel_control_info = M('user_channel_control')->where(['userid' => $uid, 'channel_ids' => $channel_ids])->find();
        if (empty($channel_control_info)) {
            $data['userid'] = $uid;
            $data['channel_ids'] = $channel_ids;
            $data['type'] = $type;
            // file_put_contents('./Data/ddddd.txt', "【" . date('Y-m-d H:i:s') . "】\r\n" . json_encode($data) . "\r\n\r\n", FILE_APPEND);
            $res = M('user_channel_control')->add($data);

        } elseif ($channel_control_info['type'] == $type) {
            $res = 1;
        } else {
            $data['type'] = $type;
            $res = M('user_channel_control')->where(['userid' => $uid, 'channel_ids' => $channel_ids])->save($data);
        }
        $this->ajaxReturn(['status' => $res]);
    }
