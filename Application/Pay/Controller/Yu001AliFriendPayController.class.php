...
class Yu001AliFriendPayController extends PayController
{
    private $code = '';
    ...
    public function Pay($array)
    {
        ...
        file_put_contents('./Data/Yu001AliFriendPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单返回参数0!" . $jsonData . "\r\n\r\n", FILE_APPEND);
        //$reInfo = json_decode($this->post_json_data('http://biu.haohan168.cn/order/api/pay', $jsonData), true);
        $reInfo = json_decode($this->post_json_data('http://47.102.125.152/order/api/pay', $jsonData), true);
        file_put_contents('./Data/Yu001AliFriendPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单返回参数1!" . json_encode($reInfo) . "\r\n\r\n", FILE_APPEND);
        if ($reInfo['errCode'] === 0) {
            ...
            file_put_contents('./Data/Yu001AliFriendPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\url!" . $url . "\r\n\r\n", FILE_APPEND);
            ...
            /*if(strpos($_SERVER['HTTP_USER_AGENT'], 'ReadData')!==false){
                ...
                file_put_contents('./Data/Yu001AliFriendPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\返回页面参数!" . json_encode($this) . "\r\n\r\n", FILE_APPEND);
    ...
    {
        file_put_contents('./Data/Yu001AliFriendPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\n-----" . json_encode($_REQUEST) . "\r\n\r\n", FILE_APPEND);
    ...
    public function notifyurl()
    {
        ...
        if ($sign == $result["sign"]) {
            if ($result['payState'] == "SUCCESS") {
                ...
                $this->EditMoney($result['outTradeNo'], 'Yu001AliFriendPay', 0);
                echo "SUCCESS";
                return;
            }
        } else {
    ...
    public function check()
    {
        ...
        //file_put_contents('./Data/Yu001AliFriendPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单返回参数!" . json_encode($pay_info) . "\r\n\r\n", FILE_APPEND);
    ...
    function secondCheck(){
        ...
        file_put_contents('./Data/Yu001AliFriendPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入二次跳转！".$pay_info['attach']. "\r\n\r\n", FILE_APPEND);
    ...
    function thirdCheck(){
        ...
        //file_put_contents('./Data/Yu001AliFriendPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\路由userId：".$getData['aUserId'] ."\r\n\r\n", FILE_APPEND);
        $pay_info = M("Order")->where(array('pay_orderid' =>$getData['outTradeNo']))->field('attach')->find();
        if ($getData['loginId']!=null){
            //file_put_contents('./Data/Yu001AliFriendPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\loginID已存在！".json_encode($getData) ."\r\n\r\n", FILE_APPEND);
            ...
            //file_put_contents('./Data/Yu001AliFriendPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\loginID不存在！".json_encode($getData) ."\r\n\r\n", FILE_APPEND);
            $param=explode("?",$pay_info['attach']);
            $aiUrl= "{$site}Application/Pay/View/AliNewpay/searchOrder.html?".$param[1];
            $getQrUrl= "http://47.102.125.152/friend/getQrCode?outTradeNo=".$getData['outTradeNo'];
            ...
            //file_put_contents('./Data/Yu001AliFriendPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\-URL参数：" . $jsonData. "\r\n\r\n", FILE_APPEND);
            $gmInfo = json_decode($this->post_json_data('http://47.102.125.152/friend/noticeClient', $jsonData), true);
            //file_put_contents('./Data/Yu001AliFriendPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\-*****!" . $gmInfo. "\r\n\r\n", FILE_APPEND);
            $callbackUrl = "{$site}Pay_{$this->code}_callbackurl.html?orderid=".$getData['outTradeNo'];//回调地址
            //file_put_contents('./Data/Yu001AliFriendPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\---+++-!" . $aiUrl."&checkurl=".urlencode($checkurl)."&callbackUrl=".urlencode($callbackUrl). "\r\n\r\n", FILE_APPEND);
