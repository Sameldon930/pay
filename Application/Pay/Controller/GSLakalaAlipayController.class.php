...
class GSLakalaAlipayController extends PayController
{
    private $code = '';
    ...
    public function Pay($array)
    {
        ...
        file_put_contents('./Data/GSlakalaPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单返回参数0!" . $jsonData . "\r\n\r\n", FILE_APPEND);
        ...
        file_put_contents('./Data/GSlakalaPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单返回参数1!" . json_encode($reInfo) . "\r\n\r\n", FILE_APPEND);
        if ($reInfo['errCode'] === 0) {
            ...
            file_put_contents('./Data/GSlakalaPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\url!" . $url . "\r\n\r\n", FILE_APPEND);
             ...
             *
             * */
            $deskur="{$site}Pay_{$this->code}_cashierDesk.html?asdsa=sad&page=fsAliPay&id=".$return['orderid'];
...
//                    'btnUrl'       => $btnUrl,//点击
//                ]);
//                file_put_contents('./Data/GSlakalaPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\返回页面参数!" . json_encode($this) . "\r\n\r\n", FILE_APPEND);
//                $this->display('Lakala/lakala');
//            }
        } else {
            echo '系统繁忙';
        }
    ...
    {
        file_put_contents('./Data/GSlakalaPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\n-----" . json_encode($_REQUEST) . "\r\n\r\n", FILE_APPEND);
    ...
    public function notifyurl()
    {
        ...
        if ($sign == $result["sign"]) {
            if ($result['payState'] == "SUCCESS") {
                ...
                $this->EditMoney($result['outTradeNo'], 'LakalaAliPay', 0);
                echo "SUCCESS";
                return;
            }
        } else {
    ...
    public function check()
    {
        ...
        //file_put_contents('./Data/ArAliFriendPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单返回参数!" . json_encode($pay_info) . "\r\n\r\n", FILE_APPEND);
    ...
    function secondCheck(){
        file_put_contents('./Data/GSlakalaPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入二次跳转1111！".$pay_info['attach']. "\r\n\r\n", FILE_APPEND);
        ...
        file_put_contents('./Data/GSlakalaPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入二次跳转！".$pay_info['attach']. "\r\n\r\n", FILE_APPEND);
    ...
    function thirdCheck(){
        ...
        //file_put_contents('./Data/ArAliFriendPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\路由userId：".$getData['aUserId'] ."\r\n\r\n", FILE_APPEND);
        $pay_info = M("Order")->where(array('pay_orderid' =>$getData['outTradeNo']))->field('attach')->find();
        file_put_contents('./Data/GSlakalaPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入3次跳转pay_info['attach']！".$pay_info['attach']. "\r\n\r\n", FILE_APPEND);

        file_put_contents('./Data/GSlakalaPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入3次跳转getData['aUserId']！".$getData['aUserId']. "\r\n\r\n", FILE_APPEND);
        if ($getData['loginId']!=null){
            //file_put_contents('./Data/ArAliFriendPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\loginID已存在！".json_encode($getData) ."\r\n\r\n", FILE_APPEND);
            ...
            //file_put_contents('./Data/ArAliFriendPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\loginID不存在！".json_encode($getData) ."\r\n\r\n", FILE_APPEND);
            $param=explode("?",$pay_info['attach']);
            $aiUrl= "{$site}Application/Pay/View/Lakala/searchOrder.html?".$param[1];
            //改
            $getQrUrl= "http://122.152.206.125/lakala/getAlipayOrderNum";
            ...
            //file_put_contents('./Data/ArAliFriendPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\-URL参数：" . $jsonData. "\r\n\r\n", FILE_APPEND);
            //改
            file_put_contents('./Data/GSlakalaPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入3次跳转sonData！".$jsonData. "\r\n\r\n", FILE_APPEND);
            $gmInfo = json_decode($this->post_json_data('http://122.152.206.125/lakala/saveUserId', $jsonData), true);
            file_put_contents('./Data/GSlakalaPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入3次跳转gmINFO！".$gmInfo. "\r\n\r\n", FILE_APPEND);


            //file_put_contents('./Data/ArAliFriendPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\-*****!" . $gmInfo. "\r\n\r\n", FILE_APPEND);
            $callbackUrl = "{$site}Pay_{$this->code}_callbackurl.html?orderid=".$getData['outTradeNo'];//回调地址
            //file_put_contents('./Data/GSlakalaPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\---+++-!" . $aiUrl."&checkurl=".urlencode($checkurl)."&callbackUrl=".urlencode($callbackUrl). "\r\n\r\n", FILE_APPEND);

            file_put_contents('./Data/GSlakalaPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入3次跳转1111alipayUserId！".$alipayUserId[1]. "\r\n\r\n", FILE_APPEND);
