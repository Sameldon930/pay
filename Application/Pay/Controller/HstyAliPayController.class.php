...
class HstyAliPayController extends PayController
{
    private $code = '';
    ...
    public function Pay($array)
    {
        ...
        if ($reInfo['errCode'] === 0) {
            $up_result=M('Order')->where(['pay_orderid' => $return['orderid']])->save(array('attach'=>$result['qrcodeUrl']));
            $deskur="{$site}Pay_{$this->code}_cashierDesk.html?page=alipay&id=".$return['orderid'];
            $returnPara = array(
                "url" => $deskur, //商户uid
            );
    ...
    public function secondCheck()
    {
        ...
        if(intval((time() - $ctime))>300){
            file_put_contents('./Data/FsWechat.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入二次查单订单超时!" . $getData['id']."-----".time() - $ctime . "\r\n\r\n", FILE_APPEND);
            header('Location: '.$callbackUrl);
            exit;

        }
        if ( $pay_info['pay_status'] == 0) {
            file_put_contents('./Data/FsWechat.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入二次查单等待支付订单!" . $getData['id'] . "\r\n\r\n", FILE_APPEND);

            $url=$pay_info['attach'];
            //$this->ajaxreturn(array('code' =>100, 'msg'=>'等待支付'));
            file_put_contents('./Data/FsWechat.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入二次查单订单超时!" . $url . "\r\n\r\n", FILE_APPEND);
            header('Location: '.$url);
        } else  {
            file_put_contents('./Data/FsWechat.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入二次查单，反复扫码!" . $getData['id'] . "\r\n\r\n", FILE_APPEND);
            header('Location: '.$callbackUrl);
        }
    }
