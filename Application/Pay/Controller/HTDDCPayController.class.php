...
class HTDDCPayController extends PayController
{
    private $code = '';
    ...
    public function Pay($array)
    {
        ...
        //file_put_contents('./Data/HTDDC.txt', "【" . date('Y-m-d H:i:s') . "】\r\正式生成订单!" . $return['orderid'] . "当前环境：".$_SERVER['HTTP_USER_AGENT'] . $return['orderid'] . "\r\n\r\n", FILE_APPEND);
        ...
        file_put_contents('./Data/HTDDC.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单参数：" . json_encode($jsonData) . "\r\n\r\n", FILE_APPEND);
        ...
        file_put_contents('./Data/HTDDC.txt', "【" . date('Y-m-d H:i:s') . "】\r\n 下单返回参数：" . json_encode($reInfo) . "\r\n\r\n", FILE_APPEND);
        if ($reInfo['errCode'] === 0) {
            ...
            $deskur="{$site}Pay_{$this->code}_cashierDesk.html?id=".$return['orderid']."&page=ddcIndex";
            //echo $second_url;."&page=ddcIndex"
            $returnPara = array(
                "url" => $deskur, //商户uid
            );
    ...
    public function notifyurl()
    {
        ...
        if ($sign == $result["sign"]) {
            if ($result['payState'] == "SUCCESS") {
                ...
                $this->EditMoney($result['outTradeNo'], 'DDC', 0);
                echo "SUCCESS";
                return;
            }
        } else {
    ...
    public function check()
    {
        ...
        //file_put_contents('./Data/HTDDC.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单返回参数!" . json_encode($getData) . "\r\n\r\n", FILE_APPEND);
        $pay_info = M("Order")->where(array('pay_orderid' =>$getData['id']))->field("pay_status")->find();
        //file_put_contents('./Data/HTDDC.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单返回参数!" . json_encode($pay_info) . "\r\n\r\n", FILE_APPEND);
    ...
    function secondCheck(){
        ...
        $pay_info = M("Order")->where(array('pay_orderid' =>$getData['id']))->field('pay_orderid,pay_amount,pay_tongdao,pay_status,attach')->find();
        file_put_contents('./Data/HTDDC.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入加好友页面!".$pay_info['attach']  . "\r\n\r\n", FILE_APPEND);
        if($pay_info['pay_status']){
            exit('您已支付此单，请勿重复支付！');
        }else if ($pay_info['attach']) {

            /*$ddcUrl= $this->_site."Application/Pay/View/AliNewpay/ddc.html?orderid="
                .$pay_info['pay_orderid']."&price=".$pay_info['pay_amount']."&platform="
                .$pay_info['pay_tongdao']."&sign=".$mysing."&orderStr=".urlencode(urlencode(urlencode(urlencode($pay_info['attach']))));
            header('Location: '.$ddcUrl);*/
            $this->assign([
                'orderid' => $pay_info['pay_orderid'],//成功跳转页面
                'price' => $pay_info['pay_amount'],//查单
                'platform' => $pay_info['pay_tongdao'],
                'sign' => $mysing,
                'orderStr' => $pay_info['attach']//实际金额
            ]);
            $this->display('AliNewpay/ddc');
        }else{
            exit('下单失败！请重试！');
        }
    }
