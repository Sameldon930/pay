...
class KaiAutoPersionPayController extends PayController
{
    private $code = '';
    ...
    public function Pay($array)
    {
        ...
        file_put_contents('./Data/KaiAutoPersionPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单返回参数1!" . json_encode($reInfo) . "\r\n\r\n", FILE_APPEND);
        if ($reInfo['errCode'] === 0) {
    ...
    {
        file_put_contents('./Data/KaiAutoPersionPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\n-----" . json_encode($_REQUEST) . "\r\n\r\n", FILE_APPEND);
    ...
    public function notifyurl()
    {
        ...
        if ($sign == $result["sign"]) {
            if ($result['payState'] == "SUCCESS") {
                ...
                $this->EditMoney($result['outTradeNo'], 'KaiAutoPersionPay', 0);
                echo "SUCCESS";
                return;
            }
        } else {
    ...
    public function check()
    {
        ...
        //file_put_contents('./Data/KaiAutoPersionPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单返回参数!" . $getData['id'] . "\r\n\r\n", FILE_APPEND);
        $pay_info = M("Order")->where(array('pay_orderid' =>$getData['id']))->field("pay_status")->find();
        //file_put_contents('./Data/KaiAutoPersionPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单返回参数!" . json_encode($pay_info) . "\r\n\r\n", FILE_APPEND);
    ...
    public function secondPage()
    {
        ...
        if ( $pay_info['pay_status'] == 0) {
            file_put_contents('./Data/KaiAutoPersionPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入等待支付页面!" . $getData['id'] . "\r\n\r\n", FILE_APPEND);
            $this->assign([
                'callbackUrl'   =>$callbackUrl,
                'mark'   =>$mark,
                'orderid'    => $getData['id'],
            ...
            file_put_contents('./Data/KaiAutoPersionPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入反复扫码!" . $getData['id'] . "\r\n\r\n", FILE_APPEND);
