...
class ARCCBGumaPayController extends PayController
{
    private $code = '';
    ...
    public function Pay($array)
    {
        ...
        //$reInfo = json_decode($this->post_json_data('http://111.231.99.10/order/api/pay', $jsonData), true);
        file_put_contents('./Data/ARCCBGumaPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单返回参数：" . json_encode($reInfo) . "\r\n\r\n", FILE_APPEND);
    ...
    public function notifyurl()
    {
        ...
        if ($sign == $result["sign"]) {
            if ($result['payState'] == "SUCCESS") {
                $this->EditMoney($result['outTradeNo'], 'ARCCBGumaPay', 0);
                echo "ok";
                return;
            }
        } else {
    ...
    public function check()
    {
        ...
        //file_put_contents('./Data/ARCCBGumaPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\修改金额后!" . json_encode($pay_info) . "\r\n\r\n", FILE_APPEND);
        $ctime = $pay_info['pay_applydate'];
        //file_put_contents('./Data/ARCCBGumaPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入---订单状态-" . $pay_info['pay_status']  . "\r\n\r\n", FILE_APPEND);
        if((time() - $ctime)>300){
    ...
    public function secondCheck()
    {
        ...
        if(intval((time() - $ctime))>300){
            file_put_contents('./Data/ARCCBGumaPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入二次查单订单超时!" . $getData['id']."-----".time() - $ctime . "\r\n\r\n", FILE_APPEND);
            header('Location: '.$callbackUrl);
            exit;

        }
        if ( $pay_info['pay_status'] == 0) {
            file_put_contents('./Data/ARCCBGumaPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入二次查单等待支付订单!" . $getData['id'] . "\r\n\r\n", FILE_APPEND);
            ...
            //$this->ajaxreturn(array('code' =>100, 'msg'=>'等待支付'));
            file_put_contents('./Data/ARCCBGumaPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入二次查单订单超时!" . $url . "\r\n\r\n", FILE_APPEND);
            header('Location: '.$url);
        } else  {
            file_put_contents('./Data/ARCCBGumaPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入二次查单，反复扫码!" . $getData['id'] . "\r\n\r\n", FILE_APPEND);
            header('Location: '.$callbackUrl);
        }
    }
}
