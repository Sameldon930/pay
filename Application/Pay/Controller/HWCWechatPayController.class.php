...
class HWCWechatPayController extends PayController
{
    private $code = '';
    ...
    public function Pay($array)
    {
       ...
       $reInfo = json_decode($this->post_json_data('http://biu.haohan168.cn/order/api/pay', $jsonData), true);
        ...
        file_put_contents('./Data/HWCWechatPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单返回参数1!" . json_encode($reInfo) . "\r\n\r\n", FILE_APPEND);
       if ($reInfo['errCode'] === 0) {
			...
			if(strpos($_SERVER['HTTP_USER_AGENT'], 'ReadData')!==false){
       		...
       		//weixin://wxpay/bizpayurl?pr=P7S3aP7
              echo createForm(
                $gateway,
                [
    ...
    {
        file_put_contents('./Data/HWCWechatPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\n-----" . json_encode($_REQUEST) . "\r\n\r\n", FILE_APPEND);
    ...
    public function notifyurl()
    {
        ...
        if ($sign == $result["sign"]) {
            if ($result['payState'] == "SUCCESS") {
                ...
                $this->EditMoney($result['outTradeNo'], 'HWCWechatPay', 0);
                echo "SUCCESS";
                return;
            }
        } else {
    ...
    public function check()
    {
        ...
        file_put_contents('./Data/HWCWechatPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单返回参数!" . $getData['id'] . "\r\n\r\n", FILE_APPEND);
        $pay_info = M("Order")->where(array('pay_orderid' =>$getData['id']))->field("pay_status")->find();
        file_put_contents('./Data/HWCWechatPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单返回参数!" . json_encode($pay_info) . "\r\n\r\n", FILE_APPEND);
