...
class HstyWechatPayController extends PayController
{
    private $code = '';
    ...
    public function Pay($array)
    {
        ...
        file_put_contents('./Data/HstyAliPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\ 加密数据：" . json_encode($native) . "\r\n\r\n", FILE_APPEND);
        ...
        $reInfo = json_decode($this->post_json_data('http://adasdsadq.natapp1.cc/order/api/pay', $jsonData), true);

        //$reInfo = json_decode($this->post_json_data('http://111.231.99.10/order/api/pay', $jsonData), true);
        file_put_contents('./Data/HstyAliPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单返回参数：" . json_encode($reInfo) . "\r\n\r\n", FILE_APPEND);
        $result = json_decode($reInfo['data'], true);
        if ($reInfo['errCode'] === 0) {
            M('Order')->where(['pay_orderid' => $return['orderid']])->save(array('attach'=>$result['qrcodeUrl']));
            file_put_contents('./Data/HstyAliPay1.txt', "【" . date('Y-m-d H:i:s') . "】\r\$result['qrcodeUrl']：" . $result['qrcodeUrl'] . "\r\n\r\n", FILE_APPEND);

            $deskur="{$site}Pay_{$this->code}_cashierDesk.html?page=wechatScan&id=".$return['orderid'];
            $returnPara = array(
                "url" => $deskur, //商户uid
            );
    ...
    public function notifyurl()
    {
        ...
        if ($sign == $result["sign"]) {
            if ($result['payState'] == "SUCCESS") {
                $this->EditMoney($result['outTradeNo'], 'FsAlipay', 0);
                echo "ok";
                return;
            }
        } else {
    ...
    public function check()
    {
        ...
        //file_put_contents('./Data/HstyPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\修改金额后!" . json_encode($pay_info) . "\r\n\r\n", FILE_APPEND);
        $ctime = $pay_info['pay_applydate'];
        //file_put_contents('./Data/HstyPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入---订单状态-" . $pay_info['pay_status']  . "\r\n\r\n", FILE_APPEND);
        if((time() - $ctime)>300){
    ...
    public function secondCheck()
    {
        //接收传输的数据
        $getData = I('get.', '');
        file_put_contents('./Data/HstyWechatPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入二次查单订单超时11!" . $getData['id']."-----".time() - $ctime . "\r\n\r\n", FILE_APPEND);

        $pay_info = M("Order")->where(array('pay_orderid' =>$getData['id']))->field('pay_status,pay_applydate,attach')->find();

        $ctime = $pay_info['pay_applydate'];
        $callbackUrl = $this->_site."Pay_{$this->code}_callbackurl.html?orderid=".$getData['id'];
        $ctime = $pay_info['pay_applydate'];
        if(intval((time() - $ctime))>300){
            file_put_contents('./Data/HstyWechatPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入二次查单订单超时!" . $getData['id']."-----".time() - $ctime . "\r\n\r\n", FILE_APPEND);
            header('Location: '.$callbackUrl);
            exit;

        }
        if ( $pay_info['pay_status'] == 0) {
            file_put_contents('./Data/HstyWechatPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入二次查单等待支付订单!" . $getData['id'] . "\r\n\r\n", FILE_APPEND);

            $url=$pay_info['attach'];
            //$this->ajaxreturn(array('code' =>100, 'msg'=>'等待支付'));
            file_put_contents('./Data/HstyWechatPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入二次查单订单超时111url!" . $url . "\r\n\r\n", FILE_APPEND);
            $this->setHtml($url,[]);
        } else  {
            file_put_contents('./Data/HstyWechatPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入二次查单，反复扫码!" . $getData['id'] . "\r\n\r\n", FILE_APPEND);
            header('Location: '.$callbackUrl);
        }
    }
