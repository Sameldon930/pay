...
class YanAutoTransferAliPayController extends PayController
{
    private $code = '';
    ...
    public function Pay($array)
    {
        ...
        file_put_contents('./Data/YanAutoTransferAliPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单返回参数：" . json_encode($reInfo) . "\r\n\r\n", FILE_APPEND);
        if ($reInfo['errCode'] === 0) {
            ...
            //$url="alipays://platformapi/startapp?appId=20000116&actionType=toAccount&goBack=NO&userId=2088332296654062&memo=&amount=2000";
            $url="{$site}Pay_{$this->code}_secondCheck.html?id=".$return['orderid']."&price=".$price."&userId=".$result['qrcodeUrl'];
            file_put_contents('./Data/YanAutoTransferAliPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\e二维码URL：" . $url . "\r\n\r\n", FILE_APPEND);

            $this->assign([
                'callbackUrl'   =>$callbackUrl,//成功跳转页面
                'orderid'    => $return['orderid'],//订单号
              'shopName'   =>$result['shopName'],//收款人
                'body'       => $body,//描述
            ...
            $this->display('AliNewpay/autoIndex');
        }else{
            echo '系统繁忙';
        }
    }
    ...
    public function notifyurl()
    {
        ...
        if ($sign == $result["sign"]) {
            if ($result['payState'] == "SUCCESS") {
                ...
                $this->EditMoney($result['outTradeNo'], 'YanAutoTransferAliPay', 0);
                echo "SUCCESS";
                return;
            }
        } else {
    ...
    public function check()
    {
        ...
        //file_put_contents('./Data/YanAutoTransferAliPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\修改金额后!" . json_encode($pay_info) . "\r\n\r\n", FILE_APPEND);
        $ctime = $pay_info['pay_applydate'];
        //file_put_contents('./Data/YanAutoTransferAliPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入---订单状态-" . $pay_info['pay_status']  . "\r\n\r\n", FILE_APPEND);
        if((time() - $ctime)>300){
    ...
    public function secondCheck()
    {
        ...
        if(intval((time() - $ctime))>300){
            file_put_contents('./Data/YanAutoTransferAliPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入二次查单订单超时!" . $getData['id']."-----".time() - $ctime . "\r\n\r\n", FILE_APPEND);
            header('Location: '.$callbackUrl);
            exit;

        }
        if ( $pay_info['pay_status'] == 0) {
            file_put_contents('./Data/YanAutoTransferAliPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入二次查单等待支付订单!" . $getData['id'] . "\r\n\r\n", FILE_APPEND);
            //$url="alipays://platformapi/startapp?appId=20000116&actionType=toAccount&goBack=NO&userId=2088332296654062&memo=&amount=2000";
            $url="alipays://platformapi/startapp?appId=20000116&actionType=toAccount&goBack=NO&userId=".$userId."&memo=&amount=".$price;
            file_put_contents('./Data/YanAutoTransferAliPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入二次查单订单超时!" . $url . "\r\n\r\n", FILE_APPEND);
            header('Location: '.$url);
        } else  {
            file_put_contents('./Data/YanAutoTransferAliPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入二次查单，反复扫码!" . $getData['id'] . "\r\n\r\n", FILE_APPEND);
            header('Location: '.$callbackUrl);
        }
    }
}
