...
class PolymerizationPayController extends PayController
{
    private $code = '';
    ...
    public function Pay($array)
    {
      ...
      //$reInfo = json_decode($this->post_json_data('http://wanma.haohan168.cn:8080/order/api/pay', $jsonData), true);
        file_put_contents('./Data/PolymerizationPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单返回参数!" . json_encode($reInfo) . "\r\n\r\n", FILE_APPEND);
        if ($reInfo['errCode'] === 0) {
             $result = json_decode($reInfo['data'], true);
         	 $return['amount']= $result['price'] / 100;
           file_put_contents('./Data/PolymerizationPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\修改金额后!" . json_encode($return) . "\r\n\r\n", FILE_APPEND);
            echo createForm(
                $gateway,
                [
    ...
    public function notifyurl()
    {
        ...
        if ($sign == $result["sign"]) {
            if ($result['payState'] == "SUCCESS") {
                ...
                $this->EditMoney($result['outTradeNo'], 'PolymerizationPay', 0);
                echo "ok";
                return;
            }
        } else {
