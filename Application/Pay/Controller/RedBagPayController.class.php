...
class RedBagPayController extends PayController
{
    private $code = '';
    ...
    public function Pay($array)
    {
        ...
        if ($reInfo['errCode'] === 0) {
            ...
            file_put_contents('./Data/'.$pay_info['pay_tongdao'].'.txt', "【" . date('Y-m-d H:i:s') . "】\r\second_url：" . $second_url . "\r\n\r\n", FILE_APPEND);
            //添加到订单的qrcode
            ...
            //file_put_contents('./Data/'.$pay_info['pay_tongdao'].'.txt', "【" . date('Y-m-d H:i:s') . "】\r\链接：" . $aliurl . "\r\n\r\n", FILE_APPEND);
    ...
    function secondCheck(){
        ...
        $pay_info = M("Order")->where(array('pay_orderid' =>$getData['id']))->field('attach,pay_status,pay_tongdao')->find();
        file_put_contents('./Data/'.$pay_info['pay_tongdao'].'.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入二次跳转!". json_encode($pay_info)  . "\r\n\r\n", FILE_APPEND);
    ...
    function thirdCheck(){
        ...
        $gmInfo = json_decode($this->post_json_data($channel['gateway'].'/redpack/saveUserMsg', $jsonData), true);
        ...
        file_put_contents('./Data/'.$pay_info['pay_tongdao'].'.txt', "【" . date('Y-m-d H:i:s') . "】\r\-jumpUrl!" . $url. "\r\n\r\n", FILE_APPEND);
        header('Location: '.$url);
