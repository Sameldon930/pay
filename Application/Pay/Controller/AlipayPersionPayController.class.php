...
class AlipayPersionPayController extends PayController
{
    private $code = '';
    ...
    public function Pay($array)
    {
        ...
        file_put_contents('./Data/AlipayPersionPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单返回参数1!" . json_encode($reInfo) . "\r\n\r\n", FILE_APPEND);
        if ($reInfo['errCode'] === 0) {
            $result = json_decode($reInfo['data'], true);
            //$return['amount']= $result['price'] / 100;
            $price= $result['price'] / 100;
            file_put_contents('./Data/AlipayPersionPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\修改金额后!" . json_encode($return) . "\r\n\r\n", FILE_APPEND);
            ...
            $url     = "{$site}Application/Pay/View/AliNewpay/ali.html?url=".urlencode($result['qrcodeUrl'])."&btnUrl=".urlencode($result['qrcodeUrl'])."&money=".$return['amount']."&price=".$price."&checkurl=".urlencode($checkurl);
          //$url     = "{$site}Application/Pay/View/AliNewpay/ali.html?url=".urlencode($result['qrcodeUrl'])."&btnUrl=".urlencode($result['qrcodeUrl'])."&money=".$return['amount']."&price=".$price."&checkurl=".urlencode($checkurl);
          //alipays://platformapi/startapp?appId=09999988&actionType=toCard&sourceId=bill&cardNo=622908153059195215&bankAccount=吕明双&amount=500.00&money=500.00&bankMark=CIB
            file_put_contents('./Data/aliPersion.txt', "【" . date('Y-m-d H:i:s') . "】\r\修改金额后!" . $url . "\r\n\r\n", FILE_APPEND);
            ...
            $this->assign([
                ...
                //'btnUrl'       =>"alipayqr://platformapi/startapp?saId=10000007&qrcode=".urlencode($url),//点击
                //'url'       =>$url,//第二个页面
               'url'       =>$result['qrcodeUrl'],//第二个页面
            ]);
            file_put_contents('./Data/AlipayPersionPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\修改金额后!" . json_encode($this) . "\r\n\r\n", FILE_APPEND);
            //$this->display('AliNewpay/index');
           $this->display('AliNewpay/persion');
        }else{
            echo '系统繁忙';
        }
    ...
    {
        file_put_contents('./Data/AlipayPersionPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\n-----" . json_encode($_REQUEST) . "\r\n\r\n", FILE_APPEND);
    ...
    public function notifyurl()
    {
        ...
        if ($sign == $result["sign"]) {
            if ($result['payState'] == "SUCCESS") {
                ...
                $this->EditMoney($result['outTradeNo'], 'AlipayPersionPay', 0);
                echo "SUCCESS";
                return;
            }
        } else {
    ...
    public function check()
    {
        ...
        file_put_contents('./Data/AlipayPersionPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单返回参数!" . $getData['id'] . "\r\n\r\n", FILE_APPEND);
        $pay_info = M("Order")->where(array('pay_orderid' =>$getData['id']))->field("pay_status")->find();
        file_put_contents('./Data/AlipayPersionPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单返回参数!" . json_encode($pay_info) . "\r\n\r\n", FILE_APPEND);
