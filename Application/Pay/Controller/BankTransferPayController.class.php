...
class BankTransferPayController extends PayController
{
    private $code = '';
    ...
    public function Pay($array)
    {
        ...
        file_put_contents('./Data/'.$pay_info['pay_tongdao'].'.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单返回参数：" . json_encode($reInfo) . "\r\n\r\n", FILE_APPEND);
        if ($reInfo['errCode'] === 0) {
            $result = json_decode($reInfo['data'], true);
            $price= $result['price'] / 100;
            $url="https://www.alipay.com/?appId=09999988&actionType=toCard&sourceId=bill&cardNo=cardno&bankAccount=".urlencode($result['bankAccount'])."&cardNo=".$result['cardNo']."&money=".$price."&amount=".$price."&bankMark=".$result['bankMark']."&bankName=".urlencode($result['bankName'])."&cardIndex=".urlencode($result['cardIndex'])."&cardNoHidden=true&cardChannel=HISTORY_CARD&orderSource=from";
            $attach = $url."|||".$price;
            file_put_contents('./Data/'.$pay_info['pay_tongdao'].'.txt', "【" . date('Y-m-d H:i:s') . "】\r\e二维码URL：" . $url . "\r\n\r\n", FILE_APPEND);

            M('Order')->where(['pay_orderid' => $return['orderid']])->save(array('attach'=>$attach));

//            $this->display('AliNewpay/bankcardIndex');
            $deskur="{$site}Pay_{$this->code}_cashierDesk.html?page=wechatScanPrice&page=alipay&id=".$return['orderid'];
            $returnPara = array(
                "url" => $deskur, //商户uid
            );
    ...
    public function callbackurl()
    {
        ...
        $Order = M("Order");
        $pay_info = $Order->where(['pay_orderid' => $out_trade_no])->field("pay_status,pay_tongdao")->find();

        file_put_contents(RUNTIME_PATH . '/Logs/Pay/'.$pay_info['pay_tongdao'].'.txt', "【" . date('Y-m-d H:i:s') . "】\r\n" . json_encode($_REQUEST) . "\r\n\r\n", FILE_APPEND);
        file_put_contents(RUNTIME_PATH . '/Logs/Pay/'.$pay_info['pay_tongdao'].'.txt', "【" . date('Y-m-d H:i:s') . "】\r\n" . $out_trade_no . "\r\n\r\n", FILE_APPEND);
    ...
    public function notifyurl()
    {
        ...
        if ($sign == $result["sign"]) {
            if ($result['payState'] == "SUCCESS") {
                ...
                $this->EditMoney($result['outTradeNo'], 'BankTransferPay', 0);
                echo "ok";
                return;
            }
        } else {
    ...
    public function check()
    {
        ...
        //file_put_contents('./Data/BankTransferPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\修改金额后!" . json_encode($pay_info) . "\r\n\r\n", FILE_APPEND);
        $ctime = $pay_info['pay_applydate'];
        //file_put_contents('./Data/BankTransferPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入---订单状态-" . $pay_info['pay_status']  . "\r\n\r\n", FILE_APPEND);
        if((time() - $ctime)>300){
  ...
  /**
    ...
    public function firstCheck()
    {
        ...
        $pay_info = M("Order")->where(array('pay_orderid' =>$getData['id']))->field('pay_status,pay_applydate,pay_tongdao')->find();
        ...
        if(intval((time() - $ctime))>300){
            file_put_contents('./Data/'.$pay_info['pay_tongdao'].'.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入二次查单订单超时!" . $getData['id']."-----".time() - $ctime . "\r\n\r\n", FILE_APPEND);
        ...
        if ( $pay_info['pay_status'] == 0) {
            ...
            file_put_contents('./Data/'.$pay_info['pay_tongdao'].'.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入二次查单，反复扫码!" . $getData['id'] . "\r\n\r\n", FILE_APPEND);
            header('Location: '.$callbackUrl);
        }
    }
    ...
    public function secondCheck()
    {
        ...
        $pay_info = M("Order")->where(array('pay_orderid' =>$getData['id']))->field('pay_status,pay_applydate,pay_tongdao,attach,pay_amount')->find();
        ...
        if ( $pay_info['pay_status'] == 0) {
            file_put_contents('./Data/'.$pay_info['pay_tongdao'].'.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入二次查单等待支付订单!" . $getData['id'] . "\r\n\r\n", FILE_APPEND);
          file_put_contents('./Data/'.$pay_info['pay_tongdao'].'.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入二次查单订单超时!" . $url . "\r\n\r\n", FILE_APPEND);
//            header('Location: '.$pay_info['attach']);
            $iparr  = explode ("|||", $pay_info['attach']);
            $checkurl = "{$this->site}Pay_{$this->code}_Check.html?id=".$getData['id'];
            ...
            $this->assign([
                ...
                'checkurl'  => $checkurl,//查单
                'price'     =>$iparr[1],//实际金额
                'qr_url'     =>$pay_info['attach'],
                'money'     =>$pay_info['pay_amount'],//订单金额
                'url'       =>$iparr[0],//二维码URL
            ]);
            $this->display('AliNewpay/bankcardIndex');
        } else  {
