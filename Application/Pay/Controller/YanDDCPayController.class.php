...
class YanDDCPayController extends PayController
{
    private $code = '';
    ...
    public function Pay($array)
    {
        ...
        file_put_contents('./Data/YanDDC.txt', "【" . date('Y-m-d H:i:s') . "】\r\正式生成订单!" . $return['orderid'] . "当前环境：".$_SERVER['HTTP_USER_AGENT'] . $return['orderid'] . "\r\n\r\n", FILE_APPEND);
        ...
        file_put_contents('./Data/YanDDC.txt', "【" . date('Y-m-d H:i:s') . "】\r\n 下单返回参数：" . json_encode($reInfo) . "\r\n\r\n", FILE_APPEND);
        if ($reInfo['errCode'] === 0) {
    ...
    public function check()
    {
        ...
        file_put_contents('./Data/YanDDC.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单返回参数!" . json_encode($getData) . "\r\n\r\n", FILE_APPEND);
        $pay_info = M("Order")->where(array('pay_orderid' =>$getData['id']))->field("pay_status")->find();
        file_put_contents('./Data/YanDDC.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单返回参数!" . json_encode($pay_info) . "\r\n\r\n", FILE_APPEND);
    ...
    function secondCheck(){
        ...
        file_put_contents('./Data/YanDDC.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入加好友页面!".$pay_info['attach']  . "\r\n\r\n", FILE_APPEND);
