...
class KaiAliRedBagPayController extends PayController
{
    private $code = '';
    ...
    public function Pay($array)
    {
        ...
        file_put_contents('./Data/KaiAliRedBagPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单返回参数1!" . json_encode($reInfo) . "\r\n\r\n", FILE_APPEND);
        if ($reInfo['errCode'] === 0) {
            $result = json_decode($reInfo['data'], true);
            //$return['amount']= $result['price'] / 100;
            $price= $result['price'] / 100;
            file_put_contents('./Data/KaiAliRedBagPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\Data参数：" . json_encode($return) . "\r\n\r\n", FILE_APPEND);
            ...
            file_put_contents('./Data/KaiAliRedBagPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\second_url：" . $second_url . "\r\n\r\n", FILE_APPEND);
            //添加到订单的qrcode
            ...
            file_put_contents('./Data/KaiAliRedBagPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\url：" . $url . "\r\n\r\n", FILE_APPEND);
            ...
            //file_put_contents('./Data/KaiAliRedBagPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\链接：" . $aliurl . "\r\n\r\n", FILE_APPEND);
            ...
            /*if(strpos($_SERVER['HTTP_USER_AGENT'], 'ReadData')!==false){
                ...
                //file_put_contents('./Data/KaiAliRedBagPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\返回页面参数!" . json_encode($this) . "\r\n\r\n", FILE_APPEND);
    ...
    {
        //file_put_contents('./Data/KaiAliRedBagPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\n-----" . json_encode($_REQUEST) . "\r\n\r\n", FILE_APPEND);
    ...
    public function notifyurl()
    {
        ...
        if ($sign == $result["sign"]) {
            if ($result['payState'] == "SUCCESS") {
                ...
                $this->EditMoney($result['outTradeNo'], 'KaiAliRedBagPay', 0);
                echo "SUCCESS";
                return;
            }
        } else {
    ...
    public function check()
    {
        ...
        file_put_contents('./Data/KaiAliRedBagPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单返回参数!" . $getData['id'] . "\r\n\r\n", FILE_APPEND);
        $pay_info = M("Order")->where(array('pay_orderid' =>$getData['id']))->field("pay_status")->find();
        //file_put_contents('./Data/KaiAliRedBagPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单返回参数!" . json_encode($pay_info) . "\r\n\r\n", FILE_APPEND);
    ...
    function secondCheck(){
        ...
        file_put_contents('./Data/KaiAliRedBagPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\进入二次跳转!". json_encode($pay_info)  . "\r\n\r\n", FILE_APPEND);
    ...
    function thirdCheck(){
        ...
        file_put_contents('./Data/KaiAliRedBagPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\-URL参数：" . $jsonData. "\r\n\r\n", FILE_APPEND);
        $gmInfo = json_decode($this->post_json_data('http://118.25.190.126/redpack/saveUserMsg', $jsonData), true);
        file_put_contents('./Data/KaiAliRedBagPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\-*****!" . $gmInfo. "\r\n\r\n", FILE_APPEND);

        $url     = "{$site}Application/Pay/View/AliNewpay/alipayAI2.html?".$param[1];
        file_put_contents('./Data/KaiAliRedBagPay.txt', "【" . date('Y-m-d H:i:s') . "】\r\-jumpUrl!" . $url. "\r\n\r\n", FILE_APPEND);
        header('Location: '.$url);
