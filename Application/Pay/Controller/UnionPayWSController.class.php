...
class UnionPayWSController extends PayController
{
    private $code = '';
    ...
    public function Pay($array)
    {
        ...
        /*$json_str='{"errCode":0,"errCodeDes":null,"resultCode":true,"data":"{\"uid\":\"5EWAYOD3GFE5X2KCNL4SD1GGC8O79CXL\",\"systemID\":\"1544780310878ji\",\"price\":\"100\",\"outTradeNo\":\"20181214173918545710\",\"overdueTime\":\"300\",\"qrcodeUrl\":\"alipays:\/\/platformapi\/startapp?appId=20000123&actionType=scan&biz_data={\"s\": \"money\",\"u\": \"2088502692116097\",\"a\": \"1.00\",\"m\": \"1544780310878ji\"}\"}"}';
        $reInfo =json_decode($json_str,true);*/

       $reInfo = json_decode($this->post_json_data('http://wanma.haohan168.cn:8080/order/api/pay', $jsonData), true);
        file_put_contents('./Data/UnionPayWS.txt', "【" . date('Y-m-d H:i:s') . "】\r\转账下单返回参数!" . json_encode($reInfo) . "\r\n\r\n", FILE_APPEND);
        if ($reInfo['errCode'] === 0) {
             $result = json_decode($reInfo['data'], true);
            /*$arrayUrl=explode('&biz_data=', $result['qrcodeUrl']);
            $biz_data=json_decode($arrayUrl[1], true);
            $userId=$biz_data['u'];
            $amount=$biz_data['a'];
            $systemID=$result['systemID'];
            //$outTradeNo=$result['outTradeNo'];
            $pamara='v=1.0.1&userId='.$userId.'&amount='.$amount.'&memo='.$systemID.'&createDate='.time();
//            $wakeipAliUrl=$this->_site.'Application/Pay/View/WeiXin/wakeupAlipay.html?'+base64_encode($pamara);
            $myurl=$this->_site.'Application/Pay/View/WeiXin/alipayH5Tranfer.html?p='.base64_encode($pamara);
            $alipayqr='alipayqr://platformapi/startapp?saId=10000007&qrcode=' .urlencode ($myurl);
            //$ds='https://ds.alipay.com/?scheme='.urlencode ($alipayqr);
            file_put_contents('./Data/WMTransferLogin.txt', "【" . date('Y-m-d H:i:s') . "】\r\n跳转链接!" . json_encode($alipayqr) . "\r\n\r\n", FILE_APPEND);
            header('Location:' . $alipayqr);*/
            echo createForm(
                $gateway,
                [
    ...
    public function notifyurl()
    {
        ...
        if ($sign == $result["sign"]) {
            if ($result['payState'] == "SUCCESS") {
                ...
                $this->EditMoney($result['outTradeNo'], 'UnionPayWS', 0);
                echo "ok";
                return;
            }
        } else {
